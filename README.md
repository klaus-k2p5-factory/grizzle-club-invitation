...
The historical generic paid-search destination remains at [`grizzl-e-club-invitation-canada/`](grizzl-e-club-invitation-canada/). Restarted paid Search traffic uses two intent-matched noindex routes instead: [`free-grizzl-e-charger-canada/`](free-grizzl-e-charger-canada/) with only `google-search-free-charger`, and [`ev-home-charging-rewards-canada/`](ev-home-charging-rewards-canada/) with only `google-search-10c-rewards` and current up-to-10¢/kWh copy. All three are excluded from `sitemap.xml`; the rewards route contains no 15¢ claim. Every secondary content page also contains a restrained disclosed card to the historical route using the fixed `organic-sitewide-free-charger` tag. Run paid-route and attribution tests with `node --test tests/ad-landing-page.test.mjs tests/paid-search-routes.test.mjs tests/sitewide-free-charger-promo.test.mjs tests/analytics.test.mjs`.
...
The named stewardship and editorial-methodology page is at [`about/`](about/). It explains Peter Mucha’s role, sourcing and correction standards, data boundaries and conditional referral compensation before its `organic-about` invitation path. Run its identity, disclosure, methodology, privacy, schema and integration tests with `node --test tests/about-page.test.mjs`.
