...
The concise, noindex paid-search destination is at [`grizzl-e-club-invitation-canada/`](grizzl-e-club-invitation-canada/). It contains only the Grizzl-E Club offer, material conditions and invitation-first consent form; it is intentionally excluded from `sitemap.xml`. Google Search traffic must use the fixed `google-search-ads` source tag. Every secondary content page also contains a restrained disclosed card to this route using the fixed `organic-sitewide-free-charger` tag. Run the route, attribution and sitewide-card tests with `node --test tests/ad-landing-page.test.mjs tests/sitewide-free-charger-promo.test.mjs tests/analytics.test.mjs`.
